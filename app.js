document.addEventListener('DOMContentLoaded', async () => {
    ...
    function deepCopy(obj) {
        return Array.isArray(obj) ? obj.map(deepCopy) : (obj && typeof obj === 'object' ? Object.fromEntries(Object.entries(obj).map(([k, v]) => [k, deepCopy(v)])) : obj);
    }

    function toSectionId(str) {
        return `category-${str.replace(/\s+/g, '-')}`;
    }

    function toSubTitleId(str) {
        return `sub-title-${str.replace(/\s+/g, '-')}`;
    }

    function getMeatChoicePrice(choice) {
        const prices = { Plain: 9.5, Beef: 12.5, Pork: 11.5, Ham: 11.25, Fish: 10.75, Chicken: 10.5 };
        return prices[choice] ?? 9.5;
    }

    function getStackChoicePrice(itemName, choice) {
        const priceMap = {
            'Banana Foster Pancake/Waffles': [5.25, 10.25],
            'Island Style Pancake/Waffles': [4.75, 9.25],
            'Banana Macadamia Pancake/Waffles': [5.0, 9.75],
            'Blueberry Pancake/Waffles': [5.0, 9.75],
            'Strawberry Pancake/Waffles': [5.0, 9.75],
            'Chocolate Pancake/Waffles': [5.0, 9.75],
            'Ube Pancake/Waffles': [4.75, 9.25],
            'Plain Pancake/Waffles': [4.25, 8.25]
        };
        const [half, full] = priceMap[itemName] || [itemName.price, itemName.price];
        return choice === 'Half Stack' ? half : full;
    }
    ...
    async function fetchMenuData() {
        ...
        try {
            ...
            currentMenuData = deepCopy(originalMenuData); // Deep copy
    ...
    function renderDesktopCategories(dataToRender = currentMenuData) {
        ...
        dataToRender.forEach(mc => mc.subCategories?.forEach(sc => {
            if (sc.items?.length) uniqueSubCategories.set(sc.category, toSectionId(sc.category));
        }));
    ...
    function renderMenuItems(dataToRender = currentMenuData) {
        ...
        dataToRender.forEach(mainCategoryData => {
            ...
            mainCategoryData.subCategories.forEach(subCategoryData => {
                ...
                section.id = toSectionId(subCategoryData.category);
                section.className = `sub-category-section ${isFirstSubCategoryInSection ? '' : 'mt-6 sm:mt-8'}`;
                section.setAttribute('aria-labelledby', toSubTitleId(subCategoryData.category));
                isFirstSubCategoryInSection = false;

                const subCategoryTitleElement = document.createElement('h2');
                subCategoryTitleElement.id = toSubTitleId(subCategoryData.category);
    ...
    function addEventListenersToItemCards() {
        ...
        if (!container.dataset.listenerAttached) {
            container.addEventListener('click', e => {
                const card = e.target.closest('.menu-item-card[data-item-id]');
                if (card) {
            ...
            container.addEventListener('keydown', e => {
                if ((e.key === 'Enter' || e.key === ' ') && e.target.classList.contains('menu-item-card')) {
                    e.preventDefault();
                    previouslyFocusedElement = e.target;
                    const itemData = findItemById(parseInt(e.target.dataset.itemId));
                    if (itemData) openDetailsModal(itemData);
                }
            });
    ...
    function applyFiltersAndSearch() {
        const searchTerm = (DOMElements.desktopSearchInput.value || DOMElements.mobileSearchInput.value || "").toLowerCase().trim();
        let tempData = deepCopy(originalMenuData); // Deep copy for filtering
    ...
    function openDetailsModal(itemDataWithCategories) {
        ...
        // Set initial price for items with choices
        const hasMeatChoices = item.meatChoices && item.meatChoices.length > 0;
        const hasStackChoices = item.stackChoices && item.stackChoices.length > 0;
        
        if (hasMeatChoices) {
            modal.itemPrice.textContent = `Price: $${getMeatChoicePrice('Plain').toFixed(2)}`;
        } else if (hasStackChoices) {
            let initialPrice = getStackChoicePrice(item.name, 'Half Stack');
            modal.itemPrice.textContent = `Price: $${initialPrice.toFixed(2)}`;
        } else {
        ...
        if (hasMeatChoices) {
            ...
            item.meatChoices.forEach((choice, index) => {
                const choiceEl = document.createElement('button');
                choiceEl.className = 'px-3 py-1 bg-gray-200 text-gray-700 rounded-full text-xs hover:bg-green-200 hover:text-green-800 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-50 transition-colors';
                choiceEl.textContent = choice;
                choiceEl.dataset.choice = choice;
                choiceEl.dataset.index = index;
                
                if (index === 0) {
                    choiceEl.classList.add('bg-green-500', 'text-white');
                    choiceEl.classList.remove('bg-gray-200', 'text-gray-700');
                }
                
                choiceEl.addEventListener('click', () => selectMeatChoice(choice, item));
                modal.meatChoicesList.appendChild(choiceEl);
            });
        }
        
        // Handle stack choices for pancakes/waffles
        if (hasStackChoices) {
            modal.meatChoicesContainer.classList.remove('hidden');
            modal.meatChoicesList.innerHTML = '';
            item.stackChoices.forEach((choice, index) => {
                const choiceEl = document.createElement('button');
                choiceEl.className = 'px-3 py-1 bg-gray-200 text-gray-700 rounded-full text-xs hover:bg-green-200 hover:text-green-800 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-50 transition-colors';
                choiceEl.textContent = choice;
                choiceEl.dataset.choice = choice;
                ...
                choiceEl.addEventListener('click', () => selectStackChoice(choice, item));
                modal.meatChoicesList.appendChild(choiceEl);
            });
        }
    ...
    function selectMeatChoice(choice, item) {
        const modal = DOMElements.detailsModal;
        const buttons = modal.meatChoicesList.querySelectorAll('button');
        ...
        const selectedButton = modal.meatChoicesList.querySelector(`[data-choice="${choice}"]`);
        if (selectedButton) {
            selectedButton.classList.add('bg-green-500', 'text-white');
            selectedButton.classList.remove('bg-gray-200', 'text-gray-700');
        }
        
        let adjustedPrice = getMeatChoicePrice(choice);
        
        console.log(`Selected: ${choice}, Price: $${adjustedPrice.toFixed(2)}, Item: ${item.name}`);
        modal.itemPrice.textContent = `Price: $${adjustedPrice.toFixed(2)}`;
    }

    function selectStackChoice(choice, item) {
        const modal = DOMElements.detailsModal;
        const buttons = modal.meatChoicesList.querySelectorAll('button');
        ...
        let adjustedPrice = getStackChoicePrice(item.name, choice);
    ...
    function setAndScrollActiveTab(subCategoryName) {
    ...
    function initializeObserver() {
        ...
        sectionObserver = new IntersectionObserver(entries => {
            ...
            if (mostVisibleEntry && mostVisibleEntry.target.id !== currentActiveSectionId) {
                ...
                if (window.innerWidth >= 640 && DOMElements.desktopCategoryTabsContainer) {
                    setAndScrollActiveTab(subCategoryName);
                }
    ...
    function getDailySpecials() {
        ...
        const specials = {
            0: { // Sunday
                items: [
                    { name: "Fried Rice Ala Spam", price: 9.00, image: "asset/images/dinner/friedrice.png" },
                    { name: "Fried Rice Ala Portuguese", price: 9.00, image: "asset/images/dinner/friedrice.png" },
                    { name: "Loco Moco", price: 13.00, image: "asset/images/breakfast/grandslam.png" },
                    { name: "Malasadas", price: 3.00, image: "asset/images/breakfast/plainpancake.png" }
                ]
            },
            1: { // Monday
                items: [
                    { name: "Grilled Chicken BBQ", price: 8.00, image: "asset/images/dinner/grilledfish.png" }
                ]
            },
            2: { // Tuesday
                items: [
                    { name: "Beef Broccoli", price: 13.00, image: "asset/images/dinner/beefmeatsoup.png" }
                ]
            },
            3: { // Wednesday
                items: [
                    { name: "Crispy Pata", price: 17.50, image: "asset/images/dinner/crispypata.png" },
                    { name: "Creamy Garlic Chicken", price: 9.00, image: "asset/images/dinner/grilledfish.png" }
                ]
            },
            4: { // Thursday
                items: [
                    { name: "Baby Back Ribs", price: 19.00, image: "asset/images/dinner/porkribssoup.png" },
                    { name: "Pork Sinigang Soup", price: 8.00, image: "asset/images/dinner/porksinigang.png" }
                ]
            },
            5: { // Friday
                items: [
                    { name: "Crispy Pork Belly", price: 13.00, image: "asset/images/dinner/crispybelly.png" },
                    { name: "Shrimp Tempura", price: 13.00, image: "asset/images/dinner/shrimpsaute.png" }
                ]
            },
            6: { // Saturday
                items: [
                    { name: "Beef Shank Soup", price: 17.50, image: "asset/images/dinner/beefmeatsoup.png" },
                    { name: "Portuguese Sausage Stirfry", price: 9.00, image: "asset/images/breakfast/portuegg.png" }
                ]
            }
        };
        return specials[dayOfWeek] || specials[0];
    }

    function updateDailySpecials() {
        const specialsSection = DOMElements.specialMenuSection;
        if (!specialsSection) return;

        const dailySpecials = getDailySpecials();
        const specialsGrid = specialsSection.querySelector('.grid');
        if (!specialsGrid) return;

        specialsGrid.innerHTML = '';
        dailySpecials.items.forEach(item => {
            const specialItem = document.createElement('div');
            specialItem.className = 'special-item-container rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300 cursor-pointer group';
            specialItem.setAttribute('role', 'listitem');
            specialItem.setAttribute('data-special-item', 'true');
            specialItem.setAttribute('data-item-name', item.name);
            specialItem.setAttribute('data-item-price', item.price);
            specialItem.setAttribute('data-item-image', item.image);
            
            specialItem.innerHTML = `
                <img src="${item.image}" alt="${item.name}" class="w-full h-36 sm:h-40 object-cover transition-all duration-300" loading="lazy">
                <div class="special-item-overlay" aria-hidden="true">
                    <div class="special-item-name">${item.name}</div>
                    <div class="special-item-price">$${item.price.toFixed(2)}</div>
                </div>
            `;
            ...
            specialItem.addEventListener('click', () => {
                const specialItemData = {
                    item: {
                        id: `special-${item.name.replace(/\s+/g, '-').toLowerCase()}`,
                        name: item.name,
                        description: `Special July menu item: ${item.name}. A delicious dish prepared with care and fresh ingredients.`,
                        price: item.price,
                        image: item.image
                    },
                    mainCategoryName: 'July Special',
                    subCategoryName: 'Daily Specials'
                };
                openDetailsModal(specialItemData);
            });
